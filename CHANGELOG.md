...
- **Settings regrouped by what they do; irreducible danger fenced off.** The old flat 16-row "Import options" card is now three cards with clear jobs: **Source** owns the input plus a collapsible **Pre-filter** (media type / date range / extensions / newer-than-last-import) that sits right before "Preview & select" so you narrow the coarse set before hand-picking; **Destination** (was Albums) owns album selection *and* the organize/tags/session-tag controls; and **Import behavior** covers Grouping and Performance & reliability. The two irreversible toggles (delete-after-import, replace-on-server) now sit in a fenced "Destructive" zone instead of buried mid-list, and auto-import/device rules moved to Source. "Check server" moved beside Start Import in the footer, where a preflight belongs.
