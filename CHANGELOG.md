...
- **Settings regrouped by what they do; irreducible danger fenced off.** The old flat 16-row "Import options" card is now three cards with clear jobs: **Source** owns auto-import/device rules, **Destination** (was Albums) owns album selection *and* the organize/tags/session-tag controls, and **Import behavior** groups the rest into Filters (collapsed by default, with an active-count badge), Grouping, and Performance & reliability. The two irreversible toggles (delete-after-import, replace-on-server) now sit in a fenced "Destructive" zone at the bottom instead of buried mid-list. "Check server" moved beside Start Import in the footer, where a preflight belongs.
