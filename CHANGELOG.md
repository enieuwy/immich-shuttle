...
- **A hand-picked import can no longer overwrite one of the photos it was asked to upload.** Two selected files whose staged names differ only by capitalisation — or only by how the source spells an accent — are two names on the card but one name in the staging folder, which is how macOS and Windows volumes work. Staging noticed the collision only by comparing the text of the two paths, so the second file was written straight through the link standing in for the first, truncating that original on the card before anything had been uploaded. Staging now asks the filesystem instead of guessing, so each selection gets its own entry, and no staging step can open a destination that already exists.
- **An import asked for exactly zero files no longer uploads the whole card.** "Check server" read such a request as nothing to do, while starting it fell back to the entire source. The request is now refused at the boundary, so the two can never disagree again.
- **"Only import media new since last time" now stops instead of quietly importing everything.** An unreadable or corrupted run history was indistinguishable from a first run, so the date floor was dropped and the full card was re-scanned without a word. A history that cannot be read is now reported, and the import does not start. A history that does not exist yet is still a first run, as before.
- **A card's file selection no longer follows the next card in.** "Clear" reset the sources but kept the selected paths, and selections are remembered by absolute path. A different card mounted at the same place with the usual camera filenames re-activated that hidden selection, so Start Import silently switched to a subset nobody had reviewed.
- **"Import again" no longer stages a run under a profile you switched to while it was working.** The profile was re-checked after each step except the last one — the source scan — so a switch during it left the previous run's source, options and delete choice on screen under the new server. A source you chose yourself after switching is left alone.
- **Quitting no longer fails because the import stopped first.** The cancel that quit sends reports that the run is no longer running when the run finishes in the moment between the two checks it makes. That answer means the quit has nothing left to stop, and it now closes the window instead of refusing.
- **Rotating the run logs no longer deletes anything else in the logs folder.** That folder is reachable from "Open Logs", and rotation kept only the newest run logs plus `app.log` — every other file in it, including anything you had put there, was eligible for deletion, and it also filled the retention budget so real run logs were rotated too early. Rotation and startup cleanup now share one rule and touch only `run-<id>.log` files.
- **A failing notification no longer surfaces as an unrelated error.** The completion and failure notices were sent without waiting for them, so a rejection from the operating system escaped the handler that was supposed to absorb it.
- **A hand-picked import now reports every selected file it could not stage.** A missing, unreadable, or colliding file was skipped before `immich-go` started, so the final card could report a clean upload even though it never offered all selected files. Staging now records each omission in the card and History. A failed copy also removes its incomplete temporary file before the uploader can see it.
- **A partial upload no longer looks complete.** If `immich-go` uploaded some files and then exited badly, or the source scan failed after some assets landed, the queue and History said "Completed" with no warning. The run still keeps that status so its landed files remain usable, but its card, receipt, and History row now state that it did not finish cleanly. An end without a confirmed process exit never advances the incremental-import checkpoint.
- **A stopped sidecar cannot escape import ownership.** A failed stop request used to consume the only process handle and let the app finish cleanup while the uploader could still run. The app now waits for a real termination event for five seconds. Without one it records an unconfirmed outcome instead of a clean cancellation, keeps the diagnostics, and never treats it as a successful import.
- **A timed-out source scan no longer becomes a complete preview.** The source picker previously committed the prefix it had seen as a normal inventory. A timeout now clears that partial selection, says what happened, and offers Retry. Cancelling a scan remains a neutral state, not an error.
- **Leaving "Check server" now stops only that check.** Changing the profile, source, selection, or filters cancels the obsolete forecast and checksum pass without cancelling the separate preview scan.
- **An auto-import rule now belongs to one physical volume.** Names such as "Untitled" and mount paths such as `/Volumes/Untitled` are reused by different cards, but rules carried both an import destination and delete-after-verify. Rules now use a proven volume id. Older name and path rules require a fresh confirmation, and never enable deletion until you confirm it for the inserted card.
- **Preview thumbnails now follow the current file, not an old file at the same path.** A restored or reused camera filename with the same length and timestamp could reuse the old thumbnail. The cache now also checks the file record and bounded content fingerprints, while unchanged files still hit the cache.
- **Run-log filenames may now contain attribute-like text.** ` file=`, ` error=`, ` reason=`, and ` discovered_at=` inside a real filename once confused the sidecar-log parser, which could drop the event or record a truncated path. Success and duplicate records now consume the terminal filename exactly.
- **New profiles require an API key, and a failed keychain check rolls back.** The editor no longer saves a profile that every server call would reject. If the keychain writes a replacement but cannot verify it, the previous secret is restored, or the new entry is removed.
- **Start Import no longer widens an incomplete scan into a whole-card import.** A timed-out or cancelled scan clears its partial file list, but Start Import then treated that empty list as "all files". It now refuses until a complete scan is available, so a preview failure cannot silently change a reviewed subset into the whole source.
- **An older Check server cleanup cannot cancel a newer check.** Forecast cancellation now belongs to the exact request generation that started it. Changing inputs while a new check begins leaves the new check running.
- **An inserted replacement card now gets a fresh identity check.** A detector cache and the card banner both treated a reused mount path as the same card. Replacing a card with another of the same size could apply the old card's destination and delete policy. The app now probes identity again and refuses a stale banner before it starts an import.
- **A completed path must now come from the import manifest, not just the run log.** A source filename with a newline could forge a success-looking log record for another file under the same folder. The app now captures the offered files before the uploader starts, and only those exact paths can become delete candidates or advance the incremental checkpoint.
...
- **Saving a profile no longer undoes the API key it just stored.** Once the new `config.json` is in place, the app asks the filesystem to confirm that the replacement itself survives a power cut. That confirmation can fail on its own — a read-only or unusual volume — and the failure was reported as though the save had never happened: the new profile stayed visible in the list while the keychain was rolled back around it, so an edited profile was left with the previous key and a new one with no key at all. A finished import was likewise told its History record could not be saved while the record was already on disk. A save that is already in place is now reported as successful, and the unconfirmed durability is recorded in the app log instead. A save that fails before it is in place still rolls the key back exactly as before.
...
- **A server that does not say whether its copy is in the trash no longer authorises deleting the original.** `isTrashed` is optional in the Immich API, and an absent field was read as "not trashed" — a proof of liveness the server never gave. A checksum whose only server copy had been moved to the trash could therefore clear the delete check, and the local original, by then the last live copy, was moved to the Trash. Only an explicit "not trashed" now counts. "Check server" still counts such a match as a duplicate, because it deletes nothing.
- **The last check before an original is deleted now proves it is the same file.** It compared length and modification time and deliberately allowed the timestamps to differ by up to a second, and fell back to length alone when a timestamp was unreadable. A file rewritten or replaced with different bytes of the same length after the server had confirmed the old ones could pass. The comparison is now exact, an unknown value refuses the delete rather than weakening it, and a file recreated at the same path is recognised as a different file.
- **A bad server response can no longer confirm the wrong original for deletion.** The bulk-upload check works in request chunks, but it trusted an echoed index from any chunk. A malformed response could therefore mark a different local row as present. The response index must now fall inside the exact request range, or the whole verification fails and keeps every file.
- **Profile URLs cannot leak embedded credentials in errors.** API normalization now removes user names and passwords with queries and fragments before an error, log entry, or request helper can format the URL.
- **Unanswered delete prompts are bounded and hold no API keys.** The app now keeps at most sixteen confirmation offers. When it withdraws the oldest offer, it keeps every source file and makes the card clearable. Confirmation reads the current profile key from the keychain instead of retaining one copy per prompt.
- **A reinserted card now explains why deletion stopped.** The final identity check stays strict after a remount. When the app can no longer prove that a file is the verified one, it leaves it in place and asks you to verify again instead of claiming that its bytes changed.
- **A sidecar that cannot be proved stopped now holds a safety lease.** The app no longer removes its staging folder or admits another overlapping import after an unconfirmed stop. It marks the run failed, keeps its sources reserved for the session, and asks for an app restart rather than guessing that the uploader exited.
- **A delete prompt now belongs to the card that earned it.** Before hashing at confirmation, the app checks every candidate against the volume identity captured when it offered the prompt. A card replaced at the same mount path cannot have its files verified or moved to the Trash by an earlier card's prompt.
- **Profile URLs now lose embedded credentials before the sidecar sees them.** Saved profiles, legacy profiles, URL selection, requests, errors, and the `immich-go` command line all use the same stripped URL.
...
- **Transitive dependency drift now has an owner.** Renovate updates what `Cargo.toml` and `package.json` declare, and its vulnerability alerts are direct-only, so a crate pulled in by Tauri or `keyring` was nobody's job — which is why all four fixes above had to be found by hand. Daily lock-file maintenance is enabled, and a refresh that pulls in an unlisted advisory fails the audit job rather than merging quietly.
...
- **A sidecar that ends without a termination event is now unconfirmed, not successful.** The run log can show that some assets landed, but it cannot prove that the uploader processed the rest or exited cleanly. The app preserves that evidence, records the missing termination, and does not advance the incremental-import checkpoint.
