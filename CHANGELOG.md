...
- **Real people on shared-album badges.** The little member badges on shared album chips now show each user's actual Immich profile picture (fetched through the authenticated backend and cached per session). Users without a picture get a colored initial matching their Immich avatar color — same identity color as the Immich web app — with a stable hash fallback for older servers. The share-with-users list in the create-album dialog shows the same avatars.
