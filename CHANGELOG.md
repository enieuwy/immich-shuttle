...
- **Settings regrouped by what they do; irreducible danger fenced off.** The old flat 16-row "Import options" card became **Source** (input + a collapsible **Pre-filter**: media type / date range / include-extensions / newer-than-last-import, plus auto-import & device rules), **Destination** (album selection + organize/tags), and a compact **Danger zone** card holding just the two irreversible toggles (delete-after-import, replace-on-server) — per-import only, never saved as defaults. "Check server" moved beside Start Import in the footer.
